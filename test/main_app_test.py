...
class MainTest(unittest.TestCase):
    ...
    def test_compute_accuracy(self):
        ...
        y_star = [1, 0, 0, 1, 0]
        y = [1, 0, 0, 1, 0]
        self.assertEqual(main_app.compute_accuracy(y_star, y), 1,
                         "Accuracy should be 1 for [1,0,0,1,0] [1,0,0,1,0]")

        # Testing full failure on 1 label
        y_star = [1]
        y = [0]
        self.assertEqual(main_app.compute_accuracy(y_star, y), 0,
                         "Accuracy should be 0 for [1] [0]")

        # Testing decimal accuracy on 5 labels
        y_star = [0, 0, 0, 0, 0]
        y = [0, 0, 0, 1, 1]
        self.assertEqual(main_app.compute_accuracy(y_star, y), Decimal('0.6'),
        ...
        self.assertEqual(main_app.compute_accuracy(y_star, y), Decimal('0.8'),
    ...
    def test_get_set(self):
        ...
        # Retrieving sets
        training_set = main_app.get_set('../dataset/1500_pairs_train.csv')
        testing_set = main_app.get_set('../dataset/400_pairs_test.csv')
    ...
    def test_normalize_set(self):
        ...
        normalized_set = main_app.normalize_set(dataset, [False, False, False, False])
